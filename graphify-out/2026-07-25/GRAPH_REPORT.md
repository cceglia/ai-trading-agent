...
## Corpus Check
- 42 files · ~20,802 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 651 nodes · 1175 edges · 50 communities (42 shown, 8 thin omitted)
...
Cohesion: 0.10
Nodes (16): If get_broker_time() fails, _analyze_structure should set fatal_error., AgentState must NOT have 'account_info'; it must have 'symbol_price' instead., AgentState must NOT have 'market_data' — the dead field was removed.      This v, The first call to decider.decide must have feedback=None.      This may already, _analyze_structure must use SnapshotBuilder to convert CSV to dicts., When only 2 of 3 TFs are cached, must fetch all fresh., test_agentstate_rejects_account_info(), test_agentstate_rejects_market_data() (+8 more)
...
Cohesion: 0.14
Nodes (13): AgentState, State for the trading graph., Conditional edge from review to decide or end., When no cache files exist, all 3 TFs must be fetched fresh., Cache-hit confluence must be the real engine confluence, not D1 analysis_context, Corrupt per-TF cache file must not crash — fall back to fresh fetch., Fresh-fetch path must also save the MTF cache file., test_analyze_structure_cache_hit_confluence_correct() (+5 more)
...
Cohesion: 0.12
Nodes (16): LangGraph orchestrator for trading analysis., TradingGraph, H1 analysis must now be saved to cache like D1/H4., get_candles must be called with broker_time param., With max_review_attempts=2, decider.decide must be called exactly 3 times     (1, With max_review_attempts=2, feedback must be forwarded to decider.decide     on, _analyze_structure must request preferred_bars for each timeframe., _analyze_structure must call get_broker_time() instead of datetime.now(UTC). (+8 more)
