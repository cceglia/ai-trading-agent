...
## Corpus Check
- 155 files · ~100,040 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3185 nodes · 7278 edges · 188 communities (155 shown, 33 thin omitted)
- Extraction: 64% EXTRACTED · 36% INFERRED · 0% AMBIGUOUS · INFERRED: 2610 edges (avg confidence: 0.64)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `8d273768`
...
Cohesion: 0.07
Nodes (50): _cache_path(), _get_cache_date(), _get_settings(), load_cached_synthesis(), Any, datetime, Path, File-based cache for SynthesizerAgent output, keyed by (symbol, day, H1-closing- (+42 more)
...
Cohesion: 0.04
Nodes (58): InvalidTradeDirectionError, Trade direction string does not map to a valid TradeDirection., Route after the review node.          - ``"continue_enforcement"`` when review i, LangGraph orchestrator for trading analysis with multi-timeframe pipeline., TradingGraph, _directional_structure_analysis(), mock_reviewer(), If get_broker_time() fails, _analyze_structure should set fatal_error. (+50 more)
...
Cohesion: 0.06
Nodes (47): Persist *summary* to a JSON cache file for *symbol* on *broker_day*.      Best-e, save_synthesis(), _canonical_structure_analysis(), _directional_structure_analysis(), _make_cached_summary(), mock_reviewer(), datetime, Path (+39 more)
...
Cohesion: 0.05
Nodes (70): DecisionAction, EnforcementViolation, EnforcementViolationCode, ExecutionStatus, FinalDecisionState, Code identifying an enforcement violation in the setup., Decision action taken by the decision agent., Status of the review process. (+62 more)
...
Cohesion: 0.13
Nodes (20): grade_setup(), Any, Grade a trading setup based on multi-timeframe structural analysis.      This fu, _d1_context(), _h1_context(), _h4_context(), Any, AAA grade when all three timeframes aligned: D1 directional,     H4 aligned cont (+12 more)
...
Cohesion: 0.18
Nodes (8): _log_llm_call(), Record an LLM call and log its cost. Returns enriched usage with costs., Tests for agent cost-logging helper extraction., _log_llm_call extracts duplicated cost-logging from agents., When usage is provided, logs cost details., When usage is all-zero (no usage data), logs zero cost., Records the call on the cost tracker when usage is provided., TestLogLlmCall

### Community 23 - "tests/orchestrator/__init__.py"
Cohesion: 0.10
Nodes (45): BaseSettings, Parse JSON string env var and validate prices.          Accepts only the new for, Resolve ``analysis_cache_dir`` to an absolute path.          Both the analyzer a, Trading agent configuration., Settings, DeterministicEnforcementGate, Enforces deterministic invariants before a decision is finalised.      The gate, _calendar_blocker() (+37 more)
...
Cohesion: 0.09
Nodes (40): Entry plan calculation for the multi-timeframe pipeline.  This module implements, Deterministic setup grading for the multi-timeframe pipeline.  This module imple, EntryType, InvalidationReason, Shared domain enums and data models for the market structure engine.  This modul, Reason a setup was invalidated., Type of entry order for a trade setup., Trade direction based on bias. (+32 more)
...
Cohesion: 0.08
Nodes (19): derive_allowed_actions(), ExecutionBlocker, Self, An execution blocker that prevents or delays trade execution.      Attributes:, Derive the allowed actions based on trade direction and execution status.      R, Derive allowed actions from direction and execution status., Create an ExecutionPolicyState from a setup and blockers.          Extracts the, Tests for enforcement logic and models (Section 16.6).  Tests the enforcement-re (+11 more)

### Community 59 - "AgentState"
Cohesion: 0.06
Nodes (32): Structure analysis missing required 'timeframes' schema., StructureSchemaError, AgentState, _has_high_impact_calendar_event(), Any, Route after early execution routing.          Returns:             - ``"determin, Validate the decision before it reaches the reviewer.          Ensures the decis, Run the deterministic enforcement gate.          The gate verifies that every ex (+24 more)
...
### Community 81 - ".write"
Cohesion: 0.21
Nodes (12): load_ohlc_cache(), datetime, OHLCBar, Save OHLC bars to cache, same directory structure as analysis cache.      Args:, Load cached OHLC bars from disk if available.      Args:         timeframe: "D1", save_ohlc_cache(), OHLCBar, Single OHLC bar for chart rendering. (+4 more)
...
Cohesion: 0.09
Nodes (14): Any, DecisionOutput, MarketContextSummary, BaseModel, Summary of market context from synthesizer agent., Decision output from decider agent.      The LLM selects an action and explains, DecisionOutput does NOT contain deterministic fields like         entry_price, s, DecisionOutput should only have symbol, action, and reasoning fields. (+6 more)
...
Cohesion: 0.08
Nodes (47): _build_blockers(), evaluate_execution_policy(), PolicySettings, Deterministic execution policy evaluation for the multi-timeframe pipeline.  Thi, Evaluate execution policy and return an :class:`ExecutionPolicyState`.      Cons, Configuration for execution policy evaluation.      Attributes:         countert, Evaluate all blocker conditions and return the active blockers.      This is an, BlockerSeverity (+39 more)

### Community 119 - "TestParseUsageChatCompletions"
Cohesion: 0.11
Nodes (14): OpenAIProviderAdapter, Any, T, OpenAI provider adapter — instructor-based structured output.  Wraps the ``instr, The provider this adapter handles., Resolved identity for the configured model., The underlying sync instructor-patched OpenAI client., The raw model identifier. (+6 more)
...
Cohesion: 0.04
Nodes (48): After D1 close (17:00), the current candle is today's., H4 period containing the given time, anchored at 00:00., At exact H1 boundary, period starts at that time., After D1 close, cache date is today's date (from period_start)., H4 cache date includes the closing hour in cache_date.hour., D1 after close with existing cache should skip analysis., D1 cache filename has no hour suffix — always d1-analysis.json., H1 should skip analysis when cache file exists for that period. (+40 more)
...
Cohesion: 0.24
Nodes (6): _get_settings(), ohlc_cache_path(), Compute OHLC cache file path, parallel to analysis cache.      Cache path conven, Tests for OHLC bar cache., Full path follows analysis/YYYY/MM/DD/SYMBOL/ pattern., TestOhlcCachePath
...
Cohesion: 0.09
Nodes (16): DeterministicSetupState, ExecutionPolicyState, BaseModel, Immutable state representing a classified trading setup.      Captures the full, Whether this setup qualifies as a candidate for decision.          A setup is a, Immutable state for execution policy evaluation.      Attributes:         trade_, Derive execution status from blockers before review stage., Evaluate all enforcement checks and produce a final decision state.          Arg (+8 more)
...
### Community 161 - "test_h4_candle_period"
Cohesion: 0.40
...
## Knowledge Gaps
- **173 isolated node(s):** `trading-ai-agent`, `create-user.sh script`, `start-dev.sh script`, `trading-server`, `*.vue` (+168 more)
...
- **Why does `Settings` connect `tests/orchestrator/__init__.py` to `usage.py`, `Mt5DataProvider`, `DataSource`, `AgentState`, `Evaluator`, `reload_settings`, `Agent Instructions`, `ResultWriterContractError`, `test_h1_candle_period`, `test_grading.py`, `TestTerminalSettings`, `tests/decision/__init__.py`, `TestGetCandlesBrokerNow`, `._run_async`, `.validate_execution_policy`, `test_h4_candle_period`, `testget_cache_date_d1_after_close`, `test_scanner.py`, `test_result_pipeline_writes_json`, `AgentState`, `test_cache_path_mtf`, `test_cache_path_mtf_uses_d1_date`, `.get_run`, `testget_cache_date_d1_before_close`?**
  _High betweenness centrality (0.288) - this node is a cross-community bridge._
- **Why does `WebSettings` connect `._run_async` to `test_runner.py`?**
  _High betweenness centrality (0.148) - this node is a cross-community bridge._
- **Why does `create_app()` connect `test_runner.py` to `test_analyze_structure_fresh_saves_mtf_cache`, `src/data/__init__.py`, `main.py`, `main.py`, `TerminalApiError`, `._run_async`, `test_d1_candle_period_after_close`?**
