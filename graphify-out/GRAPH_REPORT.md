...
## Corpus Check
- 177 files · ~104,987 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3088 nodes · 6490 edges · 186 communities (160 shown, 26 thin omitted)
...
Cohesion: 0.10
Nodes (15): BatchResponse, BaseModel, Server-specific Pydantic models., Summary of a single analysis run (schema-v2 summary contract).      Exactly: sym, Safe per-symbol terminal error envelope (§12.3).      Carries a stable diagnosti, Batch envelope returned by POST /api/run (§12.3, FR-033).      ``results`` is ke, RunSummary, SymbolError (+7 more)
...
Cohesion: 0.09
Nodes (27): AnalysisEnvelope, DecisionBlock, DeterministicFacts, OHLCBar, OHLCData, BaseModel, Single OHLC bar for chart rendering., Immutable deterministic facts for one symbol/run (Section 12.1).      ``bias`` a (+19 more)
...
Cohesion: 0.09
Nodes (20): Structure analysis missing required 'timeframes' schema., StructureSchemaError, _has_high_impact_calendar_event(), Any, Validate presentation schema only; deterministic facts remain authoritative., Run the deterministic enforcement gate.          The gate verifies that every ex, Run the trading graph for a symbol.          Args:             symbol: Trading s, Check if any calendar event has a high impact level. (+12 more)
...
Cohesion: 0.09
Nodes (26): Writes schema-v2 analysis envelopes to JSON files in the data tree., ResultWriter, _make_analysis_result(), Path, Tests for ResultWriter (schema-v2 nested envelope persistence)., A degraded analysis (synthesis failure, valid facts) must not be         rewritt, When there are errors but no fatal_error, status should be 'partial'., OHLC bars appear in the output JSON under 'ohlc'. (+18 more)
...
Cohesion: 0.09
Nodes (25): Walk the data directory tree, read/parse JSON result files,     filter/sort into, ResultScanner, Path, Legacy fatal results must not make the run list endpoint fail., Helper to write a result JSON file., Files not in YYYY/MM/DD/SYMBOL/ pattern are skipped., When JSON has a 'symbol' field, it overrides the path-derived symbol., Tests for ResultScanner.get_run(). (+17 more)
...
Cohesion: 0.06
Nodes (39): ValidationStatus, _build_blockers(), Deterministic execution policy evaluation for the multi-timeframe pipeline.  Thi, Evaluate all blocker conditions and return the active blockers.      This is an, derive_execution_status(), DeterministicSetupState, EnforcementViolation, EnforcementViolationCode (+31 more)
...
Cohesion: 0.05
Nodes (32): Any, Record the result files that exist before the run starts.          Maps each sym, Return True when *run*'s file was produced by the current batch.          A file, Build the analyzer CLI argument list., Spawn the Python process and wait for completion.          On timeout the proces, Retry reading result files with backoff.          After a subprocess completes t, Return the subset of *symbols* with no run freshly produced by the         curre, Walk the data directory via ResultScanner and return the most recent         fre (+24 more)

### Community 55 - "test_result_pipeline_writes_json"
Cohesion: 0.22
Nodes (11): DeterministicValidation, BaseModel, Result of validating deterministic facts., FinalOutputAssembler, Final decision output assembler.  This module implements :class:`FinalOutputAsse, Assembles a single :class:`AnalysisResult` from all pipeline states.      The as, Final output maps deterministic facts and validation ahead of LLM prose., _risk() (+3 more)
...
Cohesion: 0.22
Nodes (9): Select the canonical current price across timeframes.      The canonical current, Build a compact version of structure analysis suitable for LLM prompts.      The, Extract compact analytical fields from a single timeframe engine output.      Th, _select_canonical_current_price(), _summarize_structure_analysis(), _summarize_timeframe(), test_synthesizer_structure_summary_preserves_event_and_liquidity_histories(), test_synthesizer_structure_summary_preserves_invalidation_blocker() (+1 more)
...
Cohesion: 0.08
Nodes (23): BiasLevel, Structural bias levels., AdvisoryLevels, DecisionOutput, MarketContextSummary, BaseModel, Summary of market context from synthesizer agent., Optional LLM-proposed levels, never used for execution or chart overlays. (+15 more)

### Community 87 - "test_result_pipeline_writes_json"
Cohesion: 0.14
Nodes (10): LegacyAdapter, Server-owned, read-only, idempotent adapter for legacy result files.      Produc, Unit tests for ResultScanner (v2 envelopes + legacy adapter)., Minimal schema-v2 envelope mirroring the analyzer writer output., The legacy adapter is read-only, idempotent, and review-free (AC-015)., TEST-014 / AC-014: a v2 file written by the analyzer at the shared     project-r, ROOT-001 / AC-014: with a RELATIVE TRADING_ANALYSIS_CACHE_DIR the         analyz, TestLegacyAdapter (+2 more)
...
Cohesion: 0.07
Nodes (25): BaseHTTPMiddleware, FastAPI, Request, RequestResponseEndpoint, Response, create_app(), FastAPI application entry point — port of the TypeScript Express server., Resolve a provider_id to its server-side base URL (FR-039 / DEC-014).      The r (+17 more)
...
Cohesion: 0.09
Nodes (20): RunService — port of the TypeScript runner service.  Spawns the Python analyzer, ResultScanner — reads schema-v2 envelopes and adapts legacy files.  The scanner, client_with_auth(), mock_data_dir(), mock_runner(), Any, Path, RunSummary (+12 more)
...
Cohesion: 0.08
Nodes (41): evaluate_execution_policy(), PolicySettings, Evaluate execution policy and return an :class:`ExecutionPolicyState`.      Cons, Configuration for execution policy evaluation.      Attributes:         countert, BlockerSeverity, ExecutionBlockerCode, ExecutionMode, Status of the trigger confirmation process. (+33 more)
...
Cohesion: 0.09
Nodes (40): _determine_geometry_status(), Determine geometry status based on entry plan validity.      Args:         h1_se, DecisionAction, derive_allowed_actions(), EntryType, ExecutionBlockerType, ExecutionStatus, GeometryStatus (+32 more)
...
Cohesion: 0.12
Nodes (7): mock_runner(), Route-level tests with mocked scanner/runner., Sample RunSummary for route tests., Mock RunService returning an empty successful BatchResult., Tests for GET /api/runs., sample_summary(), TestListRuns
...
Cohesion: 0.09
Nodes (35): Entry plan calculation for the multi-timeframe pipeline.  This module implements, _determine_h4_aligned(), _determine_lifecycle_status(), Deterministic setup grading for the multi-timeframe pipeline.  This module imple, Determine lifecycle status based on trigger confirmation.      Args:         h1_, Determine if H4 is aligned with D1.      Args:         h4_alignment_status: H4 a, Trade direction based on bias., Quality grade assigned to a trading setup. (+27 more)
...
Cohesion: 0.30
Nodes (14): _assembled(), _enforcement(), _graph(), _policy_state(), _risk_state(), _setup_state(), test_deterministic_decision_defaults_to_no_trade_when_policy_blocks(), test_deterministic_decision_uses_policy_action_without_llm() (+6 more)
...
- **Why does `Settings` connect `DataSource` to `Evaluator`, `.test_empty_pricing_table`, `TestCostTracking`, `TestListRunsPruning`, `TestArgparseMultiSymbol`, `test_save_h4_creates_hour_suffixed_file`, `test_cache_path_zero_padded_hour`, `test_cache_path_mtf`, `test_grading.py`, `_mock_analysis_result`, `tests/data/__init__.py`, `tests/decision/__init__.py`, `main`, `test_should_run_h1_different_period`, `tests/orchestrator/__init__.py`, `setup_logging`, `TestParseUsageChatCompletions`, `._run_async`?**
  _High betweenness centrality (0.267) - this node is a cross-community bridge._
- **Why does `WebSettings` connect `TestCostTracking` to `test_runner.py`, `test_result_pipeline_writes_json`?**
  _High betweenness centrality (0.126) - this node is a cross-community bridge._
- **Why does `TradingGraph` connect `tests/data/__init__.py` to `usage.py`, `DataSource`, `Evaluator`, `Agent Instructions`, `TestDetermineLifecycleStatus`, `tests/decision/__init__.py`, `TestDetermineH1TriggerConfirmed`, `test_terminal_data_provider.py`, `TestGetCandlesBrokerNow`, `TestGetPositions`, `._run_async`, `test_load_returns_none_when_missing`, `test_save_analysis_creates_directories`, `AgentState`, `test_result_pipeline_writes_json`, `test_cache_path_d1_uses_folder_date_not_broker_now`, `test_should_run_h1_different_period`, `TestAgentApiKey`, `test_cache_path_mtf`?**
