...
## Corpus Check
- 118 files · ~59,784 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1824 nodes · 3349 edges · 135 communities (107 shown, 28 thin omitted)
- Extraction: 72% EXTRACTED · 28% INFERRED · 0% AMBIGUOUS · INFERRED: 927 edges (avg confidence: 0.69)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `20cfce30`
...
## Communities (135 total, 28 thin omitted)
...
Cohesion: 0.06
Nodes (37): _canonical_structure_analysis(), _make_cached_summary(), mock_reviewer(), datetime, Path, RED-first tests for orchestrator-level synthesizer cache integration.  Tests ver, Cache hit → cost_tracker.call_count == 0 (no LLM call recorded)., 5 sequential runs on same symbol/day → exactly 1 LLM call.          First run: c (+29 more)
...
Cohesion: 0.13
Nodes (14): main(), _parse_and_configure_settings(), Parse CLI args into a configured Settings instance.      Applies CLI overrides (, Main entry point.      Parses CLI arguments, initialises the analysis pipeline,, Warning is logged when --telegram is set but token/chat_id are empty., Verify the error log includes limit/cost details.          RED: CostLimitExceede, Test that --telegram flag triggers notifications for approved setups., With --telegram flag, notification is sent for approved buy/sell setups. (+6 more)
...
Cohesion: 0.18
Nodes (5): client(), Route-level tests with mocked scanner/runner., Tests for GET /api/runs/{symbol}/{year}/{month}/{day}/{file}., Create a test client with mocked scanner and runner., TestGetRun
...
Cohesion: 0.11
Nodes (16): BiasLevel, DecisionAction, Structural bias levels., Review verdict from reviewer agent., ReviewVerdict, TestReviewVerdict, mock_reviewer(), TestMaxReviewAttempts (+8 more)
...
Cohesion: 0.20
Nodes (7): RunService — port of the TypeScript runner service.  Spawns the Python analyzer, _mock_process(), Unit tests for RunService., Non-zero exit code should raise RuntimeError with stderr., Create a RunService with test defaults., Create a mock asyncio subprocess., runner()
...
Cohesion: 0.50
Nodes (3): _make_run_summary(), Scanner returns empty on first N-1 calls, then succeeds., Create a minimal RunSummary-like mock for the scanner.
...
## Knowledge Gaps
- **130 isolated node(s):** `trading-ai-agent`, `trading-server`, `*.vue`, `name`, `version` (+125 more)
...
- **Why does `Settings` connect `DataSource` to `setup_logging`, `Mt5DataProvider`, `AgentState`, `AgentState`, `TestProjectFiles`, `Agent Instructions`, `src/orchestrator/__init__.py`, `TestFatalError`, `tests/decision/__init__.py`, `TestGetCandlesBrokerNow`, `TestGetPositions`?**
  _High betweenness centrality (0.092) - this node is a cross-community bridge._
- **Why does `CostTracker` connect `ForexFactoryCalendar` to `setup_logging`, `setup_logging`, `test_should_run_h1_different_period`, `TestCostTracking`, `Agent Instructions`, `TestFatalError`, `tests/calendar/__init__.py`, `setup_logging`, `tests/decision/__init__.py`, `tests/__init__.py`, `tests/orchestrator/__init__.py`, `TestSynthesizeContextCanonicalPrice`, `test_h1_candle_period`, `test_h4_candle_period_at_boundary`, `test_result_pipeline_writes_json`, `test_should_run_d1_after_close_with_cache`?**
  _High betweenness centrality (0.090) - this node is a cross-community bridge._
- **Why does `TradingGraph` connect `AgentState` to `DataSource`, `Evaluator`, `TestCostTracking`, `trading-ai-agent`, `BiasLevel`, `TestFatalError`, `tests/calendar/__init__.py`, `tests/decision/__init__.py`, `test_h1_candle_period`, `test_h4_candle_period_at_boundary`, `TestGetPositions`?**
...
- **Are the 58 inferred relationships involving `AgentState` (e.g. with `Settings` and `CostLimitExceeded`) actually correct?**
  _`AgentState` has 58 INFERRED edges - model-reasoned connections that need verification._
