...
## Corpus Check
- 169 files · ~91,223 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2698 nodes · 5414 edges · 173 communities (146 shown, 27 thin omitted)
- Extraction: 70% EXTRACTED · 30% INFERRED · 0% AMBIGUOUS · INFERRED: 1620 edges (avg confidence: 0.66)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `609eb466`
...
Cohesion: 0.20
Nodes (17): EngineError, ExternalDerivedValuesError, InsufficientDataError, InvalidTradeDirectionError, ParentContextError, Any, Exception, Structure analysis missing required 'timeframes' schema. (+9 more)

### Community 11 - "Agent Instructions"
Cohesion: 0.24
Nodes (16): _action_value(), _blockers(), DeterministicValidator, _parse_timestamp(), Any, datetime, Validation of the deterministic facts contract.  This module is deliberately ind, Validate setup facts, policy facts, and their source evidence. (+8 more)
...
Cohesion: 0.17
Nodes (12): FinalDecisionState, Immutable state representing the final decision outcome.      Attributes:, Model-level assertions for deterministic enforcement output., test_enforcement_state_defaults_to_safe_action(), test_enforcement_state_serializes_violation_codes(), Output state tests for deterministic validation and enforcement., test_enforcement_violations_are_retained_for_output(), test_final_decision_state_has_no_approval_fields() (+4 more)

### Community 14 - "src/calendar/__init__.py"
Cohesion: 0.39
Nodes (6): canonical_json(), finite_number(), parse_iso_timestamp(), Any, datetime, sha256_digest()
...
Cohesion: 0.04
Nodes (46): 10. Solution Architecture and Component Responsibilities, 11. Implementation Decisions, 12.1 Canonical v2 JSON envelope, 12.2 Synthesizer response, 12.3 HTTP API, 12.4 Authentication, 12.5 Legacy adapter, 12. External and Internal Contracts (+38 more)
...
Cohesion: 0.08
Nodes (29): AgentState, _has_high_impact_calendar_event(), Any, Run the deterministic enforcement gate.          The gate verifies that every ex, Assemble the final AnalysisResult from all pipeline states., Run the trading graph for a symbol.          Args:             symbol: Trading s, Select the canonical current price across timeframes.      The canonical current, Build a compact version of structure analysis suitable for LLM prompts.      The (+21 more)

### Community 56 - "TestSynthesizeContextCanonicalPrice"
Cohesion: 0.10
Nodes (65): _build_blockers(), PolicySettings, Deterministic execution policy evaluation for the multi-timeframe pipeline.  Thi, Configuration for execution policy evaluation.      Attributes:         countert, Evaluate all blocker conditions and return the active blockers.      This is an, BlockerSeverity, DecisionAction, derive_allowed_actions() (+57 more)
...
Cohesion: 0.27
Nodes (5): _determine_lifecycle_status(), Determine lifecycle status based on trigger confirmation.      Args:         h1_, Lifecycle status of a classified setup., SetupLifecycleStatus, TestDetermineLifecycleStatus
...
### Community 81 - ".write"
Cohesion: 0.18
Nodes (15): get_cache_date(), Return the cache date for the given timeframes.      Returns a datetime whose da, load_ohlc_cache(), OHLCBar, Save OHLC bars to cache, same directory structure as analysis cache.      Args:, Load cached OHLC bars from disk if available.      Args:         timeframe: "D1", save_ohlc_cache(), OHLCBar (+7 more)
...
Cohesion: 0.18
Nodes (14): DeterministicValidation, BaseModel, Result of validating deterministic facts., DecisionOutput, Decision output from decider agent.      The LLM selects an action and explains, FinalOutputAssembler, Final decision output assembler.  This module implements :class:`FinalOutputAsse, Assembles a single :class:`AnalysisResult` from all pipeline states.      The as (+6 more)
...
Cohesion: 0.09
Nodes (19): BaseHTTPMiddleware, FastAPI, Request, RequestResponseEndpoint, Response, create_app(), FastAPI application entry point — port of the TypeScript Express server., Create and configure the FastAPI application. (+11 more)
...
Cohesion: 0.27
Nodes (14): get_profile(), build_confluence(), build_timeframe_context(), _direction_from_bias(), Any, _require_parent(), analyze_multi_timeframe(), analyze_snapshot() (+6 more)
...
Cohesion: 0.07
Nodes (24): create_llm_client(), LLMClientProtocol, LLMCommunicationClient, OpenAIProviderAdapter, Protocol, LLM client protocol, provider adapters, and factory for structured LLM calls.  T, Raised when an unsupported LLM provider is requested., OpenAI provider adapter implementing ``LLMClientProtocol``.      Wraps an ``inst (+16 more)
...
Cohesion: 0.16
Nodes (14): Writes analysis results to JSON files in the data/ directory tree., ResultWriter, _make_analysis_result(), Path, Tests for ResultWriter., When there are errors but no fatal_error, status should be 'partial'., OHLC bars appear in the output JSON under 'ohlc'., ResultWriterContractError when analysis_result is missing. (+6 more)
...
Cohesion: 0.18
Nodes (9): BiasLevel, Structural bias levels., AdvisoryLevels, MarketContextSummary, BaseModel, Summary of market context from synthesizer agent., Optional LLM-proposed levels, never used for execution or chart overlays., TestDecisionOutput (+1 more)
...
Cohesion: 0.10
Nodes (20): ValidationStatus, Entry plan calculation for the multi-timeframe pipeline.  This module implements, _determine_h4_aligned(), _determine_trade_direction(), Deterministic setup grading for the multi-timeframe pipeline.  This module imple, Determine trade direction from D1 bias and H4 preferred direction.      Args:, Determine if H4 is aligned with D1.      Args:         h4_alignment_status: H4 a, EntryType (+12 more)
...
Cohesion: 0.06
Nodes (34): AnthropicModelIdentityResolver, GenericAliasModelIdentityResolver, LLMModelConfig, ModelIdentityResolver, OpenAIModelIdentityResolver, ProviderKind, Protocol, StrEnum (+26 more)
...
Cohesion: 0.62
Nodes (6): _bar(), Any, _swing(), test_failed_breakout_remains_history_and_later_confirmation_is_latest(), test_level_lifecycle_exposes_policy_fields(), test_liquidity_history_survives_reclaim_and_later_acceptance()
...
Cohesion: 0.60
Nodes (5): _graph(), test_deterministic_decision_defaults_to_no_trade_when_policy_blocks(), test_deterministic_decision_uses_policy_action_without_llm(), test_graph_has_no_secondary_llm_nodes(), test_synthesis_receives_deterministic_facts()
...
## Knowledge Gaps
- **223 isolated node(s):** `trading-ai-agent`, `create-user.sh script`, `start-dev.sh script`, `trading-server`, `*.vue` (+218 more)
...
- **Why does `Settings` connect `DataSource` to `Mt5DataProvider`, `.test_empty_pricing_table`, `ResultWriterContractError`, `TestCostTracking`, `TestArgparseMultiSymbol`, `TestDetermineD1Directional`, `test_cache_path_mtf`, `test_grading.py`, `_mock_analysis_result`, `setup_logging`, `tests/decision/__init__.py`, `TestAgentApiKey`, `test_should_run_h1_different_period`, `TestSynthesizeContextCanonicalPrice`, `test_result_pipeline_writes_json`, `Architecture`, `test_result_pipeline_writes_json`?**
  _High betweenness centrality (0.266) - this node is a cross-community bridge._
- **Why does `TradingGraph` connect `test_result_pipeline_writes_json` to `DataSource`, `test_cache_path_d1_uses_folder_date_not_broker_now`, `orchestrator/test_synthesizer_cache.py`, `Agent Instructions`, `AgentState`, `trading-ai-agent`, `test_cache_path_d1_uses_folder_date_not_broker_now`, `.write`, `tests/decision/__init__.py`, `test_should_run_h1_different_period`, `test_result_pipeline_writes_json`, `TestSynthesizeContextCanonicalPrice`, `test_terminal_data_provider.py`, `TestGetCandlesBrokerNow`, `TestGetPositions`?**
  _High betweenness centrality (0.148) - this node is a cross-community bridge._
- **Why does `create_app()` connect `test_runner.py` to `server/tests/conftest.py`, `TestCostTracking`, `test_analyze_structure_fresh_saves_mtf_cache`, `src/data/__init__.py`, `main.py`, `TerminalApiError`?**
