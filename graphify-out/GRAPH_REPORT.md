...
## Corpus Check
- 172 files · ~101,523 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3024 nodes · 6376 edges · 185 communities (158 shown, 27 thin omitted)
- Extraction: 70% EXTRACTED · 30% INFERRED · 0% AMBIGUOUS · INFERRED: 1941 edges (avg confidence: 0.66)
...
Cohesion: 0.06
Nodes (17): BatchStatus, BatchResult, Per-symbol terminal outcomes for one batch run (FR-033 / INV-014).      ``result, Batch status per FR-033: success / partial / error.          ``success`` when al, Tests for POST /api/run — batch envelope (FR-033, AC-016/020)., Symbols are normalized once for request keys and analyzer args., NFR-006: xauusd + XAUUSD are the same symbol and run once., Dedup preserves first-occurrence order. (+9 more)
...
Cohesion: 0.08
Nodes (21): BatchResponse, BaseModel, Server-specific Pydantic models., Summary of a single analysis run (schema-v2 summary contract).      Exactly: sym, Request body for POST /api/run.      Accepts 1–20 validated symbols plus optiona, Bound model id length/format (NFR-004 input hygiene)., Safe per-symbol terminal error envelope (§12.3).      Carries a stable diagnosti, Batch envelope returned by POST /api/run (§12.3, FR-033).      ``results`` is ke (+13 more)
...
Cohesion: 0.07
Nodes (41): MarketContextSummary, Summary of market context from synthesizer agent., AnalysisEnvelope, AnalysisResult, DecisionBlock, DeterministicFacts, EntryPlan, OHLCBar (+33 more)
...
Cohesion: 0.11
Nodes (15): _dict_to_sns(), make_raw_response(), Recursively convert a dict to a SimpleNamespace., Build a mock provider response with controlled usage fields.      The returned o, Primary field names: input_tokens / output_tokens., input_tokens_details = None must not crash., output_tokens_details = None must not crash., cached_input_tokens > input_tokens → clamped to input, uncached = 0. (+7 more)
...
Cohesion: 0.09
Nodes (37): grade_setup(), Any, Grade a trading setup based on multi-timeframe structural analysis.      This fu, DecisionAction, Decision action taken by the decision agent., Structured rejection codes for deterministic setup classification., SetupRejectionCode, _d1_context() (+29 more)

### Community 20 - "tests/data/__init__.py"
Cohesion: 0.09
Nodes (25): AgentState, _has_high_impact_calendar_event(), Any, Validate presentation schema only; deterministic facts remain authoritative., Run the deterministic enforcement gate.          The gate verifies that every ex, Run the trading graph for a symbol.          Args:             symbol: Trading s, Select the canonical current price across timeframes.      The canonical current, Build a compact version of structure analysis suitable for LLM prompts.      The (+17 more)
...
Cohesion: 0.08
Nodes (31): Exception, Map a stored action to the canonical v2 decision action (DEC-002).      ``wait_f, Writes schema-v2 analysis envelopes to JSON files in the data tree., Raised when ResultWriter receives an invalid or incomplete result., _resolve_action(), ResultWriter, ResultWriterContractError, _make_analysis_result() (+23 more)
...
Cohesion: 0.06
Nodes (45): DeterministicValidation, BaseModel, Result of validating deterministic facts., DeterministicSetupState, EnforcementViolation, ExecutionPolicyState, FinalDecisionState, BaseModel (+37 more)
...
Cohesion: 0.12
Nodes (9): FR-033 / INV-014 / AC-016 — per-symbol terminal outcomes., One symbol missing → the other symbol's result is retained (AC-016)., Missing/malformed result is a safe error, never operational success., FR-033 status rules: success / partial / error., A fatal ``error`` result file is not a result — its symbol gets a         per-sy, BATCH-001: a symbol with only a stale prior file and a current fatal         fai, BATCH-001: one stale-only symbol fails while a fresh symbol succeeds         → b, BATCH-001: a file produced during the current run is operational         success (+1 more)

### Community 54 - "main.py"
Cohesion: 0.06
Nodes (31): Spawn Python subprocess to run analysis, enforce timeout,     capture stderr, an, RunService, _make_run_summary(), _mock_process(), Unit tests for RunService., Verify --model flag is absent when model is None., A resolved provider base_url is passed as --base-url to the analyzer., Verify --base-url is absent when base_url is None. (+23 more)
...
Cohesion: 0.08
Nodes (48): evaluate_execution_policy(), PolicySettings, Deterministic execution policy evaluation for the multi-timeframe pipeline.  Thi, Evaluate execution policy and return an :class:`ExecutionPolicyState`.      Cons, Configuration for execution policy evaluation.      Attributes:         countert, BlockerSeverity, ExecutionBlockerCode, ExecutionBlockerType (+40 more)
...
Cohesion: 0.08
Nodes (23): BaseHTTPMiddleware, FastAPI, Request, RequestResponseEndpoint, Response, create_app(), FastAPI application entry point — port of the TypeScript Express server., Resolve a provider_id to its server-side base URL (FR-039 / DEC-014).      The r (+15 more)
...
Cohesion: 0.09
Nodes (12): RunService — port of the TypeScript runner service.  Spawns the Python analyzer, Tests for authentication and rate-limiting middleware., Tests for AuthMiddleware behaviour via client_with_auth., Tests that exceptions are logged before re-raising., list_runs must log the original exception before raising RuntimeError., get_run must log the original exception before raising RuntimeError., run_analysis must log the original exception before raising RuntimeError., run_analysis must not have a dedicated 502 path: a TimeoutError that         esc (+4 more)
...
Cohesion: 0.07
Nodes (25): create_llm_client(), LLMClientProtocol, LLMCommunicationClient, OpenAIProviderAdapter, Exception, Protocol, LLM client protocol, provider adapters, and factory for structured LLM calls.  T, Raised when an unsupported LLM provider is requested. (+17 more)

### Community 106 - "TestListRunsPruning"
Cohesion: 0.13
Nodes (14): _compact_timeframe_facts(), Any, datetime, OHLCBar, Path, Write a success/degraded/partial schema-v2 envelope to disk.          Fatal pipe, Validate and return the flat internal AnalysisResult for a run., Build the validated schema-v2 envelope from the flat result. (+6 more)
...
Cohesion: 0.16
Nodes (11): _build_blockers(), Evaluate all blocker conditions and return the active blockers.      This is an, derive_execution_status(), ExecutionBlocker, Self, An execution blocker that prevents or delays trade execution.      Attributes:, Derive the execution status from a set of blockers.      Priority order (highest, Derive execution status from the active blockers. (+3 more)
...
Cohesion: 0.18
Nodes (5): ProviderKind, Return ``True`` if this resolver can handle *model*., Resolve *model* into an :class:`LLMModelIdentity`., Supported LLM provider identifiers., BaseException
...
Cohesion: 0.07
Nodes (42): ValidationStatus, Entry plan calculation for the multi-timeframe pipeline.  This module implements, _determine_trade_direction(), Deterministic setup grading for the multi-timeframe pipeline.  This module imple, Determine trade direction from D1 bias and H4 preferred direction.      Args:, derive_allowed_actions(), EnforcementViolationCode, EntryType (+34 more)
...
Cohesion: 0.08
Nodes (11): mock_runner(), Route-level tests with mocked scanner/runner., Tests for GET /api/runs/{symbol}/{year}/{month}/{day}/{file}., Sample RunSummary for route tests., Malformed date/file components are rejected at the route boundary         before, Traversal attempts are absorbed by URL normalization or rejected;         in no, Mock RunService returning an empty successful BatchResult., Tests for GET /api/runs. (+3 more)
...
Cohesion: 0.09
Nodes (14): _log_llm_call(), Any, LLM agents for the trading pipeline.  The synthesizer is the only LLM integratio, Record an LLM call and log its cost. Returns enriched usage with costs., Synthesizes market context from structure analysis and calendar., SynthesizerAgent, test_synthesizer_uses_injected_client_once(), test_synthesizer_prompt_treats_deterministic_facts_as_authoritative() (+6 more)
...
- **Why does `Settings` connect `DataSource` to `Evaluator`, `.test_empty_pricing_table`, `TestCostTracking`, `AgentState`, `TestListRunsPruning`, `TestArgparseMultiSymbol`, `test_cache_path_zero_padded_hour`, `test_cache_path_d1_uses_folder_date_not_broker_now`, `test_grading.py`, `_mock_analysis_result`, `TestDetermineH1ChochBased`, `tests/decision/__init__.py`, `test_should_run_h1_different_period`, `tests/data/__init__.py`, `tests/orchestrator/__init__.py`, `setup_logging`, `TestParseUsageChatCompletions`?**
  _High betweenness centrality (0.278) - this node is a cross-community bridge._
- **Why does `TradingGraph` connect `tests/data/__init__.py` to `DataSource`, `test_load_returns_none_when_missing`, `test_save_analysis_creates_directories`, `Evaluator`, `orchestrator/test_synthesizer_cache.py`, `Agent Instructions`, `TestDetermineLifecycleStatus`, `test_cache_path_d1_uses_folder_date_not_broker_now`, `tests/calendar/__init__.py`, `tests/decision/__init__.py`, `test_should_run_h1_different_period`, `test_terminal_data_provider.py`, `TestGetCandlesBrokerNow`, `TestGetPositions`, `._run_async`?**
