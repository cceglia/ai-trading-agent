...
## Corpus Check
- 118 files · ~59,726 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1824 nodes · 3347 edges · 117 communities (103 shown, 14 thin omitted)
- Extraction: 72% EXTRACTED · 28% INFERRED · 0% AMBIGUOUS · INFERRED: 925 edges (avg confidence: 0.69)
...
Cohesion: 0.06
Nodes (38): _canonical_structure_analysis(), _make_cached_summary(), mock_reviewer(), datetime, Path, RED-first tests for orchestrator-level synthesizer cache integration.  Tests ver, Cache hit → cost_tracker.call_count == 0 (no LLM call recorded)., Cache miss → cost_tracker.call_count == 1 (one LLM call recorded). (+30 more)
...
Cohesion: 0.05
Nodes (33): Retry reading result files with backoff.          After a subprocess completes t, Return the subset of *symbols* that have no run in the scanner., Walk the data directory via ResultScanner and return the         most recent res, Spawn Python subprocess to run analysis, enforce timeout,     capture stderr, an, Run analysis for the given symbols.          Spawns: python main.py --output-dir, Spawn the Python process and wait for completion.          On timeout the proces, RunService, _make_run_summary() (+25 more)
...
Cohesion: 0.09
Nodes (19): RunService — port of the TypeScript runner service.  Spawns the Python analyzer, client(), client_with_auth(), mock_data_dir(), Any, Path, RunSummary, Shared fixtures for server tests. (+11 more)
...
- **Why does `Settings` connect `DataSource` to `setup_logging`, `Mt5DataProvider`, `AgentState`, `Evaluator`, `AgentState`, `TestProjectFiles`, `TestFatalError`, `tests/decision/__init__.py`, `TestOhlcCachePath`, `TestGetCandlesBrokerNow`, `TestGetPositions`?**
