...
## Corpus Check
- 179 files · ~107,385 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3166 nodes · 6617 edges · 200 communities (173 shown, 27 thin omitted)
- Extraction: 70% EXTRACTED · 30% INFERRED · 0% AMBIGUOUS · INFERRED: 1967 edges (avg confidence: 0.66)
...
Cohesion: 0.09
Nodes (19): _dict_to_sns(), make_raw_response(), Recursively convert a dict to a SimpleNamespace., Reset the _settings sentinel in synthesizer_cache before each test.      Mirrors, Build a mock provider response with controlled usage fields.      The returned o, reset_synthesizer_cache_settings(), sample_decision(), sample_market_context() (+11 more)
...
Cohesion: 0.11
Nodes (10): CostLimitExceeded, Exception, CostTracker — tracks LLM API call costs.  Exposes a single :class:`CostTracker`, Raised when per-symbol LLM cost exceeds the configured limit., Record an LLM API call and return its usage with cost filled in.          Parame, CostLimitExceeded is a subclass of Exception., record_call raises CostLimitExceeded when total_cost > limit., set_limit() after calls already recorded does not retroactively raise. (+2 more)
...
Cohesion: 0.07
Nodes (34): BiasLevel, DecisionAction, derive_allowed_actions(), Decision action taken by the decision agent., Structural bias levels., Derive the allowed actions based on trade direction and execution status.      R, Derive allowed actions from direction and execution status., AdvisoryLevels (+26 more)
...
Cohesion: 0.11
Nodes (10): Tests for CostTracker — tracks LLM API call costs.  CostTracker lives in ``src/d, CostTracker: tracks LLM API call costs., Many calls accumulate total_cost correctly., After set_limit(0.05), internal _limit reflects the value., total_cost == limit does NOT raise (strict > comparison)., set_symbol('XAUUSD') stores symbol internally., set_symbol('B') after set_symbol('A') overwrites symbol., CostTracker() with no args uses empty pricing → zero costs. (+2 more)
...
Cohesion: 0.12
Nodes (15): Synchronous variant of :meth:`generate_structured`.          Returns ``(response, Synchronous variant of :meth:`generate_structured`.          Returns ``(response, LLMUsage, parse_usage(), Extract an ``LLMUsage`` from a provider response.      Handles:     * ``None`` r, Immutable record of token usage for a single LLM API call.      Token fields are, Tests for usage.py — LLMUsage, safe_non_negative_int, and parse_usage.  No exter, Fallback field names: prompt_tokens / completion_tokens. (+7 more)
...
Cohesion: 0.12
Nodes (12): evaluate_execution_policy(), PolicySettings, Evaluate execution policy and return an :class:`ExecutionPolicyState`.      Cons, Configuration for execution policy evaluation.      Attributes:         countert, _make_risk_policy(), _make_setup(), Calendar has highest priority among non-execution blockers., When multiple blocker types present, status uses the highest priority. (+4 more)
...
Cohesion: 0.21
Nodes (10): _calculate_entry_plan_inner(), _extract_entry_prices(), Any, Entry plan calculation for the multi-timeframe pipeline.  This module implements, Extract and normalize entry price data from setup context.      Args:         se, Inner implementation that may raise InvalidTradeDirectionError., InvalidTradeDirectionError, Trade direction string does not map to a valid TradeDirection. (+2 more)
...
Cohesion: 0.08
Nodes (9): mock_runner(), Route-level tests with mocked scanner/runner., Tests for GET /api/runs/{symbol}/{year}/{month}/{day}/{file}., Malformed date/file components are rejected at the route boundary         before, Traversal attempts are absorbed by URL normalization or rejected;         in no, Mock RunService returning an empty successful BatchResult., Tests for GET /api/runs., TestGetRun (+1 more)
...
Cohesion: 0.09
Nodes (45): ValidationStatus, Trade direction based on bias., Quality grade assigned to a trading setup., Status of the setup classification process., Lifecycle status of a classified setup., SetupClassificationStatus, SetupGrade, SetupLifecycleStatus (+37 more)
...
Cohesion: 0.26
Nodes (4): _determine_trade_direction(), Deterministic setup grading for the multi-timeframe pipeline.  This module imple, Determine trade direction from D1 bias and H4 preferred direction.      Args:, TestDetermineTradeDirection

### Community 177 - ".test_record_call_does_not_raise_when_below_limit"
Cohesion: 0.10
Nodes (10): Unknown model logs warning, preserves token fields, cost fields zero., Missing cached_input_per_million → cached_input_cost = 0.0., Zero tokens result in zero cost but call IS counted., CostTracker(pricing={}) — record_call warns and returns zero costs., total_cost < limit does NOT raise., set_limit(-1) disables enforcement., Default CostTracker without set_limit() never raises., Build an LLMUsage with computed uncached and total. (+2 more)
...
Cohesion: 0.10
Nodes (8): Static assets / SPA fallback are served through the proxy, not /api., CORS-001: in enforced mode, OPTIONS /api/* preflight is protected.          ``Au, An authenticated preflight reaches CORSMiddleware and gets CORS         headers,, Tests for AuthMiddleware behaviour via client_with_auth.      Auth is enforced w, When TRADING_API_KEY and TRADING_TRUSTED_PROXY_CIDRS are empty         (default), AC-017: GET /api routes reject missing credentials when enforced., §15: a 401 on POST /api/run must never reach the runner., TestAuthMiddleware
...
- **Why does `Settings` connect `DataSource` to `Evaluator`, `.test_empty_pricing_table`, `TestCostTracking`, `Agent Instructions`, `TestDetermineLifecycleStatus`, `TestArgparseMultiSymbol`, `test_grading.py`, `_mock_analysis_result`, `tests/data/__init__.py`, `tests/decision/__init__.py`, `tests/orchestrator/__init__.py`, `test_routes.py`, `test_cache_path_zero_padded_hour`, `WebSettings`, `test_result_pipeline_writes_json`, `TestGenericAliasModelIdentityResolver`, `.validate_execution_policy`, `setup_logging`, `test_should_run_h1_different_period`, `TestListRunsPruning`?**
