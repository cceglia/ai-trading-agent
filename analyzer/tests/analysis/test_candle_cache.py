...
def test_cache_path_d1_uses_folder_date_not_broker_now():
    """D1 file path uses folder_date from get_cache_date, not raw broker_now."""
...
def test_cache_path_h4_includes_closing_hour():
    ...
    cache_date = get_cache_date("H4", broker_now)
...
def test_cache_path_h1_includes_closing_hour():
    ...
    broker_now = datetime(2026, 7, 21, 14, 30)
    cache_date = get_cache_date("H1", broker_now)
...
def test_cache_path_zero_padded_hour():
    ...
    cache_date_h4 = get_cache_date("H4", broker_now)
    path_h4 = _cache_path("H4", "XAUUSD", cache_date_h4)
    assert "h4-04-analysis.json" in path_h4

    cache_date_h1 = get_cache_date("H1", broker_now)
...
# =============================================================================
# RED Tests for get_cache_date
# =============================================================================


def testget_cache_date_d1_before_close(monkeypatch):
...
def test_cache_path_mtf(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TRADING_ANALYSIS_CACHE_DIR", str(tmp_path / "analysis"))
    broker_now = datetime(2026, 7, 21, 14, 0)
    cache_date = get_cache_date("MTF", broker_now)
...
def test_cache_path_mtf_uses_d1_date(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TRADING_ANALYSIS_CACHE_DIR", str(tmp_path / "analysis"))
    broker_now = datetime(2026, 7, 21, 14, 0)
    mtf_cache_date = get_cache_date("MTF", broker_now)
    d1_cache_date = get_cache_date("D1", broker_now)
...
def test_settings_has_analysis_cache_dir(monkeypatch):
    ...
    assert s.analysis_cache_dir == "data"
